...
>
> ⚠️ **(2026-06-24) 이 문서는 옛 등록 계획서다. 루틴 지시문·판정 로직의 현재 정본은 [`cloud-routine-setup.md`](./cloud-routine-setup.md).** 아래의 `[AI초안]` 제목 접두·커리큘럼 단위 판정은 **더 이상 쓰지 않는다** → 판정은 **반별 + 그 날짜기록의 콘텐츠 존재 여부(contentIds)**, 제목은 내용에 맞게(접두 없음). 옛 표현은 기록용으로만 남겨둠.
