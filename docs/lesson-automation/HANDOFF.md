...
## 2. 확정된 설계 결정
- ★ **2026-07-03 인터리브 개편**: 1 회차(수업 1회, 보통 2시수) = **이론 덱 1개(19장, 회차당) + 개념당 실습 1개(3~6개, 각 약 8~10분)**. 수업은 덱을 켜 둔 채 "이론 1~3장 → 그 개념의 실습 → 다음 개념 1~3장 → 실습 …"으로 짧게 끊어 진행. 1교시/2교시 분리 없음(쉬는 시간은 교사 판단). 덱의 각 개념 마지막 장에 「✅ 개념 끝 → 🛠 실습: {실습 제목}」 배지 = 교사가 실습 여는 신호. 개념 순서 = 날짜기록 contentIds 순서 = 배지 순서. 통역(교사 음성 실시간 번역)이 돌아가므로 슬라이드 정보 밀도 OK(그림만 고집 X). 의미 없는 필러 장 금지, 지난 시간 상기 장은 이어지는 내용일 때만. *(구모델 "시수당 1이론(16장)+1실습(20분)"은 폐기.)*
- **이론 = NotebookLM (수동)**. NotebookLM은 공개 API가 없어 자동화 제외. **역할 분리(2026-07-03)**: 디자인·형식·언어 고정 소스 = `notebooklm-guidelines.md`(§1 순수 디자인, §2 형식·언어), **19장 내용 구성 정본 = `cloud-routine-setup.md` 5단계**(1장 표지 / 2~3장 도입부 / 4~19장 개념 블록 16장 + 배지 위치). 회차별 프롬프트는 새벽 루틴이 자동 생성(수동 폴백 = 채팅 프로젝트 `{회차}회차`).
...
→ **실습 1개 = 개념 1개 = 약 8~10분**(2026-07-03 개편 — 회차당 3~6개를 이론과 번갈아 진행). 짧아도 스테이지 2~3개 × 2~3라운드로 밀도 유지.
...
- **C. 새벽 클라우드 루틴.** (전체에서 선정된 1회차) → `details` 파싱 → **개념(주요 활동)마다 실습 HTML 생성 + 회차당 이론 19장 프롬프트 1개**(2026-07-03 개편, `cloud-routine-setup.md` 정본) → `create_practice_content`(title = 개념 내용에 맞게, 회차/시수·접두 없음) → 반환 id를 **`upsert_lesson_record`로 그 반의 그 날짜(반별 `sessionStates[sessionId].date`) "날짜별 수업기록" `contentIds`에 넣기**(= 강사 화면에 바로 선택됨, `curriculumSessionId` 연결). ★ **한 번 실행에 딱 1회차**(가장 가까운 빈 회차 1개)만 — 반마다 1개씩 아님. ★ **반별 독립 — 같은 커리큘럼이라도 반마다 새로 생성**(다른 반 자료 재사용·공유 안 함). **학생 공개 아님**(`publishedLessons` 안 건드림). (선택) `mutate_curriculum_sessions`로 회차 plan contentIds도 동기화. **공개하지 않음.** 팀 이 계정 토큰으로, **MCP+키로만** 접속(로컬 비밀 X).
...
- `docs/lesson-automation/notebooklm-guidelines.md` — 이론(NotebookLM) 고정 소스 ①순수 디자인 ②형식·언어. (19장 내용 구성·회차별 프롬프트 정본은 `cloud-routine-setup.md` 5단계.)
- `docs/lesson-automation/chat-project-guide.md` — 채팅 프로젝트 지침: 커리큘럼 설계·수정(details 형식 = 개념 3~6개) + 수업 자료 생성·검수·수정(루틴과 같은 흐름·템플릿).
