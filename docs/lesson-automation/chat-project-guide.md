...
> **★ 2026-07-03 인터리브 개편**: 수업은 이론 덱(회차당 1개, **19장**)을 켜 둔 채 **1~3장 설명 → 그 개념의 실습 → 다음 개념 1~3장 → 실습 …**으로 진행한다(1교시/2교시 분리 없음, 쉬는 시간은 교사 판단). 실습은 **시수당 1개가 아니라 개념당 1개**(회차당 3~6개, 각 약 8~10분). 이론 덱의 각 개념 마지막 장에 「✅ 개념 끝 → 🛠 실습: {실습 제목}」 배지가 있어 교사가 그 실습을 연다. **개념 순서 = 날짜기록 contentIds 순서 = 배지 순서**. 이론 프롬프트는 회차당 19장 1개([`notebooklm-guidelines.md`](./notebooklm-guidelines.md) §4 정본).
...
선택한 회차(현재 과정 기준):
1. 그 회차 `details` 파싱 → **개념 목록** 도출: 시수 블록들을 순서대로 합쳐 '주요 활동' 항목들을 개념으로(1시수 활동들 → 2시수 활동들 순, 적정 3~6개 — 많으면 묶고 적으면 나눔). 표: `# | 개념 제목 | 출처 시수 | 관련 결과물`.
2. **기존 실습 HTML**: 그 커리큘럼을 쓰는 한 반의 그 날짜 날짜기록 `contentIds`로 `get_resource(contents, <id>)`를 가져와 **개념 순서대로 content id + 제목 + description + previewUrl** 표시(contentIds 순서 = 개념 순서. 실물은 html 전문을 채팅에 붓지 말고 previewUrl로 본다). → 이 id들을 **작업 대상으로 기억**(아래 ③④에서 재검색 없이 사용).
3. **회차당 NotebookLM 19장 프롬프트 1개** 생성. 디자인·형식(§1·§2, **병기 번역 레이아웃·개념 끝 배지 포함**)은 NotebookLM 소스에 이미 고정돼 있다고 전제하고, [`notebooklm-guidelines.md`](./notebooklm-guidelines.md) **§4의 19장 템플릿**에 {회차}·{회차주제}·{개념 목록(개념 제목 + 그 개념 실습 제목)}·{이어지는 내용 여부}를 채우고, **`{병기언어}`에는 그 반의 `annotationLanguages`(get_overview의 병기 언어 목록 — 강사가 클래스 설정에서 직접 지정, 0개~여러 개; 없으면 빈칸)** 를 넣어 **템플릿 전문을 그대로 출력**한다(짧게 줄이지 말 것). ⚠️ 슬라이드엔 학생이 직접 따라 하는 **실습 단계·완료 확인 기준을 넣지 말 것** — 순수 이론, 각 개념 마지막 장의 「✅ 개념 끝 → 🛠 실습」 배지로 연결만. 강사는 이 프롬프트를 그대로 NotebookLM 슬라이드 생성 칸에 붙인다.
   > 참고: 평소엔 **새벽 루틴이 이 19장 프롬프트를 자동 생성**해 대시보드 '이론 수업'에 프롬프트(복사 버튼)로 띄운다([`cloud-routine-setup.md`](./cloud-routine-setup.md)). 이 명령은 **수동 생성·검수·수정용 폴백**이며, §4 템플릿과 형식을 동일하게 맞춘다.
4. 그 반·그 회차 날짜기록에 실습 콘텐츠가 **없으면** "이 회차는 아직 실습이 없어요. 만들까요?" → 승인 시 `practice-recipe.md`대로 **개념당 1개(약 8~10분)씩** 생성 후 ④ 등록(개념 순서 유지). (같은 커리큘럼 다른 반에 이미 있으면 새로 만들지 말고 그 content id를 연결.)
