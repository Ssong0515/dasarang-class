...
const hasLegacyContentLinks = (lesson) =>
  (Array.isArray(lesson.contentIds) && lesson.contentIds.length > 0) ||
  (typeof lesson.contentId === 'string' && lesson.contentId.trim().length > 0);
...
const clearWithAdminSdk = async () => {
  ...
  const legacyLessons = snapshot.docs.filter((lessonDoc) => hasLegacyContentLinks(lessonDoc.data()));

  for (const lessonChunk of chunk(legacyLessons, BATCH_SIZE)) {
    const batch = adminDb.batch();
    lessonChunk.forEach((lessonDoc) => {
      batch.set(lessonDoc.ref, { contentId: '', contentIds: [] }, { merge: true });
    });
    await batch.commit();
  }
  ...
  return {
    ...
    clearedLessons: legacyLessons.length,
    mode: 'admin-sdk',
  };
};

const clearWithAdminLogin = async () => {
  ...
  const clientApp = initializeClientApp(firebaseConfig, `legacy-cleanup-${Date.now()}`);
  ...
  for (const lessonChunk of chunk(legacyLessons, BATCH_SIZE)) {
    ...
    lessonChunk.forEach((lessonDoc) => {
      batch.set(clientDoc(clientDb, 'lessons', lessonDoc.id), { contentId: '', contentIds: [] }, { merge: true });
    });
    await batch.commit();
  }
...
try {
  const result = getFirebaseServiceAccount()
    ? await clearWithAdminSdk()
    : await clearWithAdminLogin();
